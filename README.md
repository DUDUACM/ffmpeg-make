...
FFmpeg 多平台构建系统（GitHub Actions，全在线依赖）。支持 **macOS + Windows + Linux(Ubuntu) + Android**，纯 LGPL 配置（可闭源动态链接）。
...
| Windows | x86_64 / arm64 | ubuntu-24.04 交叉 | GCC MinGW-w64（x86_64）+ llvm-mingw（arm64） | D3D11VA/DXVA2 + NVENC/CUVID + Vulkan |
...
### Windows（D3D11VA/DXVA2 + CUVID/NVENC + Vulkan）
...
| `script/android/` | Android 交叉脚本（NDK，3 ABI） |
...
- `platforms`：`macos,windows,linux,android` 的逗号子集（留空 = 全部）
...
例如: build/ffmpeg-9.0-macos-arm64/
      ...
      build/ffmpeg-9.0-windows-x86_64/
```

Windows 产物含 `libffmpeg.dll` + `lib/libffmpeg.dll.a` + `lib/libffmpeg.def`。
...
> - **Windows**：`libffmpeg.dll` 只依赖 Windows 系统 DLL（d3d11/dxva2/kernel32 等，系统自带）；MinGW 运行期已静态链入。NVENC/CUVID 需 NVIDIA 显卡 + 驱动。
